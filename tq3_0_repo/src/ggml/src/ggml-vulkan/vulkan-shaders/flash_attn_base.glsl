...
// Extract 4 consecutive 3-bit indices starting at position iqs within a block
// iqs = value index within the block (0..31), must be aligned to 4
FLOAT_TYPEV4 dequantize4(uint ib, uint iqs, uint a_offset, uint binding_idx) {
    // Each group of 8 values is packed into 3 bytes
    // iqs gives the starting value index
    uint group = iqs / 8u;
    uint within = iqs % 8u;
    uint base = group * 3u;
    ...
    FLOAT_TYPE scale;
    if (binding_idx == BINDING_IDX_K) {
        // Read 3 bytes as packed16: byte_base maps to uint16 indices
        // qs[12] as uint16[6]: qs16[0] = qs[0]|(qs[1]<<8), qs16[1] = qs[2]|(qs[3]<<8), etc.
    ...
    // 24-bit combined value for easier extraction
    uint bits24 = b0 | (b1 << 8) | (b2 << 16);

    // Extract 4 consecutive 3-bit indices starting at 'within'
    uint shift = within * 3u;
    uint idx0 = (bits24 >> shift) & 7u;
    uint idx1 = (bits24 >> (shift + 3u)) & 7u;
    uint idx2 = (bits24 >> (shift + 6u)) & 7u;
    uint idx3 = (bits24 >> (shift + 9u)) & 7u;

    return scale * FLOAT_TYPEV4(
        tq3_centroids_fa[idx0],
        tq3_centroids_fa[idx1],
        tq3_centroids_fa[idx2],
        tq3_centroids_fa[idx3]
    );
}
#endif
