...
const std::vector<ggml_type> kv_cache_types = {
    ...
    GGML_TYPE_TQ3_0,
    GGML_TYPE_TQ3_2,
    // GGML_TYPE_TQ3_3 intentionally disabled: the dedicated Vulkan shader
    // variant exhibits a ~15% throughput penalty vs TQ3_2 that persists even
    // when the GLSL body is made byte-identical to the TQ3_2 path (verified
    // in bench_tq3_3_baseline.json). Root cause is below the GLSL layer
    // (SPIR-V compiler or driver pipeline artifact). A REAL TQ3_3 requires
    // a distinct storage format (e.g. 18 B / 64-element shared-scale V
    // blocks) plus CPU quantize/dequantize/vec_dot and a Vulkan FA kernel
    // that tolerates mismatched K/V block sizes — outside single-session
    // scope. Re-enable once the real architecture lands.
    // GGML_TYPE_TQ3_3,
    GGML_TYPE_TQ2_0,
};
