...
// TQ3_2: same block format as TQ2_0 — enhanced decode in FA shader
void quantize_row_tq3_2(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(k % QK_TQ3_2 == 0);
    block_tq3_2 * GGML_RESTRICT y = vy;
    quantize_row_tq3_2_ref(x, y, k);
}
...
// TQ3_2: same dot product as TQ2_0 (difference is FA shader enhanced V decode)
void ggml_vec_dot_tq3_2_q8_K_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
