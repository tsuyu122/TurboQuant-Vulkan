...
// 2.0625 bpw (block size 32 for better scale precision)
#define QK_TQ2_0 32
typedef struct {
    uint8_t qs[QK_TQ2_0/4]; // 2 bits per element, 32*2=64 bits = 8 bytes
    ggml_half d;
} block_tq2_0;
...
typedef struct {
    ...
    uint8_t qs[QK_TQ3_0*3/8]; // 3 bits per element, packed: 32*3=96 bits = 12 bytes
} block_tq3_0;
static_assert(sizeof(block_tq3_0) == sizeof(ggml_half) + QK_TQ3_0*3/8, "wrong tq3_0 block size/padding");

// 2.0625 bpw - Enhanced TQ2_0 with recency-aware V decode in flash attention
#define QK_TQ3_2 32
typedef struct {
    uint8_t qs[QK_TQ3_2/4]; // 2 bits per element, 32*2=64 bits = 8 bytes (same as TQ2_0)
    ggml_half d;
} block_tq3_2;
static_assert(sizeof(block_tq3_2) == sizeof(ggml_half) + QK_TQ3_2/4, "wrong tq3_2 block size/padding");

// TQ3_3 — storage identical to TQ3_2 (10 B/block); compute-only differences
// (Lloyd-Max V centroids + cross-quad decorrelation) applied in Vulkan FA kernel.
#define QK_TQ3_3 QK_TQ3_2
typedef block_tq3_2 block_tq3_3;
