...
void quantize_row_tq2_0_ref(const float * GGML_RESTRICT x, block_tq2_0 * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t i = 0; i < nb; i++) {
        ...
        const float d = amax;
        // 4-level uniform codebook: {-1, -1/3, +1/3, +1} × d
        // id = 1.5/d maps [-d,d] → [-1.5, 1.5]; adding 1.5 gives [0,3]
        const float id = d ? 1.5f/d : 0.0f;

        y[i].d = GGML_FP32_TO_FP16(d);

        // Pack 4 values per byte (2 bits each): 32 values = 8 bytes
        for (int b = 0; b < QK_TQ2_0/4; b++) {
            uint8_t q = 0;
            for (int n = 0; n < 4; n++) {
                int xi = (int)lroundf(x[b*4 + n] * id + 1.5f); // [−d,d]→[0,3]
                xi = xi < 0 ? 0 : (xi > 3 ? 3 : xi);
                q |= (uint8_t)(xi & 3) << (2*n);
            }
            y[i].qs[b] = q;
...
// TQ3_2: same block format as TQ2_0, but scale `d` is chosen by an 8-candidate
// MSE grid search instead of amax (~50% per-block MSE reduction on realistic
// V distributions while keeping byte layout and decode path identical).
void quantize_row_tq3_2_ref(const float * GGML_RESTRICT x, block_tq3_2 * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TQ3_2 == 0);
    const int64_t nb = k / QK_TQ3_2;

    static const float kFactors[8] = {1.00f, 0.90f, 0.82f, 0.75f, 0.68f, 0.62f, 0.56f, 0.50f};

    for (int64_t i = 0; i < nb; ++i) {
        float amax = 0.0f;
        for (int j = 0; j < QK_TQ3_2; ++j) {
        ...
        for (int s = 0; s < 8; ++s) {
            ...
            const float id    = d_try > 0.0f ? 1.5f / d_try : 0.0f;
            const float step  = d_try / 3.0f;
            float err = 0.0f;
            for (int j = 0; j < QK_TQ3_2; ++j) {
                int q = (int)lroundf(x[j] * id + 1.5f);
                if (q < 0) q = 0; else if (q > 3) q = 3;
                const float dec = (float)(q * 2 - 3) * step;
                const float e   = x[j] - dec;
                err += e * e;
            }
            if (err < best_err) {
                best_err = err;
        ...
        const float id = d > 0.0f ? 1.5f / d : 0.0f;

        y[i].d = GGML_FP32_TO_FP16(d);

        for (int b = 0; b < QK_TQ3_2 / 4; ++b) {
            uint8_t q_byte = 0;
            for (int n = 0; n < 4; ++n) {
                int q = (int)lroundf(x[b*4 + n] * id + 1.5f);
                if (q < 0) q = 0; else if (q > 3) q = 3;
                q_byte |= (uint8_t)(q & 3) << (2*n);
            }
            y[i].qs[b] = q_byte;
        }
        x += QK_TQ3_2;
...
void dequantize_row_tq3_2(const block_tq3_2 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    dequantize_row_tq2_0((const block_tq2_0 *)x, y, k);
}

size_t quantize_tq3_2(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights;
    const size_t row_size = ggml_row_size(GGML_TYPE_TQ3_2, n_per_row);
    quantize_row_tq3_2_ref(src, dst, (int64_t)nrow*n_per_row);
    return nrow * row_size;
}
...
void dequantize_row_tq2_0(const block_tq2_0 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t i = 0; i < nb; ++i) {
        ...
        for (int b = 0; b < QK_TQ2_0/4; b++) {
            for (int n = 0; n < 4; n++) {
                int q = (x[i].qs[b] >> (n*2)) & 3;
                // 4-level codebook: idx*2-3 gives {-3,-1,+1,+3}; divide by 3
                *y++ = (float)(q * 2 - 3) * (d / 3.0f);
            }
        }
    }
}
