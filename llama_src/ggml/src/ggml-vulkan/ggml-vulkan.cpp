...
struct vk_device_struct {
    ...
    // Mixed KV: K=TQ3_0 + V=TQ2_0
    std::map<vk_fa_pipeline_state, vk_pipeline> pipeline_flash_attn_tq3k_tq2v;

    // Mixed KV: K=TQ3_0 + V=TQ3_2 (enhanced V decode with recency window)
    std::map<vk_fa_pipeline_state, vk_pipeline> pipeline_flash_attn_tq3k_tq3_2v;

    // Mixed KV: K=TQ3_0 + V=TQ3_3 (Lloyd-Max V centroids + cross-quad decorrelation)
    std::map<vk_fa_pipeline_state, vk_pipeline> pipeline_flash_attn_tq3k_tq3_3v;
...
static vk_fa_tuning_params get_fa_tuning_params_scalar(const vk_device& device, uint32_t hsk, uint32_t hsv, uint32_t n_rows, uint32_t n_kv, ggml_type kv_type, bool f32acc) {
    ...
    // AMD RDNA: use a wider KV tile for ternary types at large context to amortize dequant overhead.
    // shmem_staging is already enabled for RDNA non-GCN; the fallback below auto-disables it if Bc is too large.
    if (device->vendor_id == VK_VENDOR_ID_AMD && device->architecture != AMD_GCN &&
        (kv_type == GGML_TYPE_TQ3_0 || kv_type == GGML_TYPE_TQ3_2 || kv_type == GGML_TYPE_TQ3_3 || kv_type == GGML_TYPE_TQ2_0) && n_kv >= 32768) {
        if (n_rows == 1) {
            result.block_cols = 128;   // 64 → 128
        } else if (!(D & 8)) {
            result.block_cols = 64;    // 32 → 64 for multi-row prefill
        }
    }
    ...
    result.shmem_staging = ((device->vendor_id == VK_VENDOR_ID_NVIDIA ||
        (device->vendor_id == VK_VENDOR_ID_AMD && device->architecture != AMD_GCN))
        && hsk < 256 && hsv < 256) ? 1 : 0;

    // TQ types use semi-quantized compute — kvsh[] staging is compile-time excluded in the shader,
    // so allocating the shared memory would be pure waste that reduces occupancy.
    if (kv_type == GGML_TYPE_TQ3_0 || kv_type == GGML_TYPE_TQ3_2 || kv_type == GGML_TYPE_TQ3_3 || kv_type == GGML_TYPE_TQ2_0) {
    ...
    // Fallback: disable shmem_staging if it doesn't fit in shared memory
    if (result.shmem_staging && !ggml_vk_flash_attn_scalar_shmem_support(device, result, hsk, hsv, f32acc)) {
        result.shmem_staging = 0;
    }
...
static vk_fa_tuning_params get_fa_tuning_params_coopmat1(const vk_device& device, uint32_t hsk, uint32_t hsv, uint32_t n_rows, uint32_t n_kv, ggml_type kv_type, bool f32acc) {
    ...
    // Fallback: disable shmem_staging if it doesn't fit in shared memory
    if (result.shmem_staging && !ggml_vk_flash_attn_coopmat_shmem_support(device, result, hsk, hsv, f32acc)) {
        result.shmem_staging = 0;
    }
...
static vk_fa_tuning_params get_fa_tuning_params(const vk_device& device, uint32_t hsk, uint32_t hsv, uint32_t n_rows, uint32_t n_kv, ggml_type kv_type, bool f32acc) {
    ...
    // TQ types: semi-quantized compute only implemented in FA_SCALAR path
    if ((kv_type == GGML_TYPE_TQ3_0 || kv_type == GGML_TYPE_TQ3_2 || kv_type == GGML_TYPE_TQ3_3 || kv_type == GGML_TYPE_TQ2_0) && path != FA_SCALAR) {
        path = FA_SCALAR;
    }
...
#define CREATE_FA(TYPE, NAMELC, FAPATH, SUFFIX) \
    ...
    if (device->fp16) {
        ...
        CREATE_FA(GGML_TYPE_TQ3_2, tq3_2, FA_SCALAR, )
        CREATE_FA(GGML_TYPE_TQ3_3, tq3_3, FA_SCALAR, )
        CREATE_FA(GGML_TYPE_TQ2_0, tq2_0, FA_SCALAR, )
    } else {
        ...
        CREATE_FA(GGML_TYPE_TQ3_2, tq3_2, FA_SCALAR, _fp32)
        CREATE_FA(GGML_TYPE_TQ3_3, tq3_3, FA_SCALAR, _fp32)
        CREATE_FA(GGML_TYPE_TQ2_0, tq2_0, FA_SCALAR, _fp32)
    }
    ...
    if (device->coopmat1_fa_support) {
        ...
        // TQ types excluded from CM1 — semi-quantized compute only in FA_SCALAR
    }
#endif
    ...
    if (device->coopmat2) {
        ...
        // TQ types excluded from CM2 — semi-quantized compute only in FA_SCALAR
    }
#endif
#undef CREATE_FA

    // Mixed KV: K=TQ3_0 + V=TQ2_0 (scalar path only)
    for (auto &fa : device->pipeline_flash_attn_tq3k_tq2v) {
        FaCodePath path = fa.first.path;
        uint32_t Br = fa.first.Br;
        uint32_t Bc = fa.first.Bc;
        bool aligned = fa.first.aligned;
        ...
        if (path == FA_SCALAR) {
            if (aligned) {
                if (f32acc) {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_f32acc_tq3k_tq2v", flash_attn_f32_f16_tq3k_tq2v_len, flash_attn_f32_f16_tq3k_tq2v_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                } else {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_f16acc_tq3k_tq2v", flash_attn_f32_f16_tq3k_tq2v_f16acc_len, flash_attn_f32_f16_tq3k_tq2v_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                }
            } else {
                if (f32acc) {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_f32acc_tq3k_tq2v", flash_attn_f32_f16_tq3k_tq2v_len, flash_attn_f32_f16_tq3k_tq2v_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                } else {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_f16acc_tq3k_tq2v", flash_attn_f32_f16_tq3k_tq2v_f16acc_len, flash_attn_f32_f16_tq3k_tq2v_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                }
            }
        }
    }

    // Mixed KV: K=TQ3_0 + V=TQ3_2 (enhanced V decode, scalar path only)
    for (auto &fa : device->pipeline_flash_attn_tq3k_tq3_2v) {
        FaCodePath path = fa.first.path;
        uint32_t Br = fa.first.Br;
        uint32_t Bc = fa.first.Bc;
        bool aligned = fa.first.aligned;
        ...
        if (path == FA_SCALAR) {
            if (aligned) {
                if (f32acc) {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_f32acc_tq3k_tq3_2v", flash_attn_f32_f16_tq3k_tq3_2v_len, flash_attn_f32_f16_tq3k_tq3_2v_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                } else {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_f16acc_tq3k_tq3_2v", flash_attn_f32_f16_tq3k_tq3_2v_f16acc_len, flash_attn_f32_f16_tq3k_tq3_2v_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                }
            } else {
                if (f32acc) {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_f32acc_tq3k_tq3_2v", flash_attn_f32_f16_tq3k_tq3_2v_len, flash_attn_f32_f16_tq3k_tq3_2v_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                } else {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_f16acc_tq3k_tq3_2v", flash_attn_f32_f16_tq3k_tq3_2v_f16acc_len, flash_attn_f32_f16_tq3k_tq3_2v_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                }
            }
        }
    }

    // Mixed KV: K=TQ3_0 + V=TQ3_3 (Lloyd-Max V centroids + cross-quad decorrelation, scalar path only)
    for (auto &fa : device->pipeline_flash_attn_tq3k_tq3_3v) {
        FaCodePath path = fa.first.path;
        uint32_t Br = fa.first.Br;
        uint32_t Bc = fa.first.Bc;
        bool aligned = fa.first.aligned;
        ...
        if (path == FA_SCALAR) {
            if (aligned) {
                if (f32acc) {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_f32acc_tq3k_tq3_3v", flash_attn_f32_f16_tq3k_tq3_3v_len, flash_attn_f32_f16_tq3k_tq3_3v_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                } else {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_f16acc_tq3k_tq3_3v", flash_attn_f32_f16_tq3k_tq3_3v_f16acc_len, flash_attn_f32_f16_tq3k_tq3_3v_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                }
            } else {
                if (f32acc) {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_f32acc_tq3k_tq3_3v", flash_attn_f32_f16_tq3k_tq3_3v_len, flash_attn_f32_f16_tq3k_tq3_3v_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                } else {
                    ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_f16acc_tq3k_tq3_3v", flash_attn_f32_f16_tq3k_tq3_3v_f16acc_len, flash_attn_f32_f16_tq3k_tq3_3v_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, (!fa_ds), ((!fa_ds) ? fa_sgs : 0));
                }
            }
        }
    }
...
#undef CREATE_MM
    ...
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_TQ3_0],   "dequant_tq3_0",   dequant_tq3_0_len,   dequant_tq3_0_data,   "main", 2, 5 * sizeof(uint32_t), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_TQ3_2],   "dequant_tq3_2",   dequant_tq3_2_len,   dequant_tq3_2_data,   "main", 2, 5 * sizeof(uint32_t), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_TQ2_0],   "dequant_tq2_0",   dequant_tq2_0_len,   dequant_tq2_0_data,   "main", 2, 5 * sizeof(uint32_t), {256, 1, 1}, {}, 1);
    ...
    ggml_vk_create_pipeline(device, device->pipeline_get_rows[GGML_TYPE_TQ3_2],   "get_rows_tq3_2",   get_rows_tq3_2_len,   get_rows_tq3_2_data,   "main", 3, sizeof(vk_op_binary_push_constants), {1024, 1, 1}, {}, 1);
    ...
    ggml_vk_create_pipeline(device, device->pipeline_get_rows_f32[GGML_TYPE_TQ3_2],   "get_rows_tq3_2_f32",   get_rows_tq3_2_f32_len,   get_rows_tq3_2_f32_data,   "main", 3, sizeof(vk_op_binary_push_constants), {1024, 1, 1}, {}, 1);
    ...
    if (device->float_controls_rte_fp16) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_TQ3_2], "cpy_f32_tq3_2", cpy_f32_tq3_2_rte_len, cpy_f32_tq3_2_rte_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_TQ2_0], "cpy_f32_tq2_0", cpy_f32_tq2_0_rte_len, cpy_f32_tq2_0_rte_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
    } else {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_TQ3_2], "cpy_f32_tq3_2", cpy_f32_tq3_2_len, cpy_f32_tq3_2_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_TQ3_3], "cpy_f32_tq3_3", cpy_f32_tq3_3_len, cpy_f32_tq3_3_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
...
#define SET_ROWS(itype, rte) \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_set_rows ## itype [GGML_TYPE_TQ3_2], "set_rows_tq3_2" #itype, set_rows_tq3_2 ## itype ## rte ## _len, set_rows_tq3_2 ## itype ## rte ## _data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true); \
        ggml_vk_create_pipeline(device, device->pipeline_set_rows ## itype [GGML_TYPE_TQ3_3], "set_rows_tq3_3" #itype, set_rows_tq3_3 ## itype ## rte ## _len, set_rows_tq3_3 ## itype ## rte ## _data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true); \
        ggml_vk_create_pipeline(device, device->pipeline_set_rows ## itype [GGML_TYPE_TQ2_0], "set_rows_tq2_0" #itype, set_rows_tq2_0 ## itype ## rte ## _len, set_rows_tq2_0 ## itype ## rte ## _data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
...
#undef SET_ROWS
    ...
    ggml_vk_create_pipeline(device, device->pipeline_cpy_quant_f32[GGML_TYPE_TQ3_2], "cpy_tq3_2_f32", cpy_tq3_2_f32_len, cpy_tq3_2_f32_data, "main", 2, sizeof(vk_op_unary_push_constants), {(uint32_t)ggml_blck_size(GGML_TYPE_TQ3_2), 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_cpy_quant_f32[GGML_TYPE_TQ3_3], "cpy_tq3_3_f32", cpy_tq3_3_f32_len, cpy_tq3_3_f32_data, "main", 2, sizeof(vk_op_unary_push_constants), {(uint32_t)ggml_blck_size(GGML_TYPE_TQ3_3), 1, 1}, {}, 1);
...
static vk_pipeline ggml_vk_get_to_fp16(ggml_backend_vk_context * ctx, ggml_type type) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_TQ3_2:
        case GGML_TYPE_TQ3_3:
...
static void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, ggml_tensor * dst) {
    ...
    // Mixed KV supported: K=TQ3_0 + V=TQ2_0, V=TQ3_2, or V=TQ3_3
    assert(k->type == v->type || (k->type == GGML_TYPE_TQ3_0 && (v->type == GGML_TYPE_TQ2_0 || v->type == GGML_TYPE_TQ3_2 || v->type == GGML_TYPE_TQ3_3)));

    const bool mixed_kv = (k->type == GGML_TYPE_TQ3_0 && v->type == GGML_TYPE_TQ2_0);
    const bool mixed_kv_tq3_2 = (k->type == GGML_TYPE_TQ3_0 && v->type == GGML_TYPE_TQ3_2);
    const bool mixed_kv_tq3_3 = (k->type == GGML_TYPE_TQ3_0 && v->type == GGML_TYPE_TQ3_3);
...

    {
        ...
        auto &pipelines = mixed_kv_tq3_3
            ? ctx->device->pipeline_flash_attn_tq3k_tq3_3v
            : mixed_kv_tq3_2
            ? ctx->device->pipeline_flash_attn_tq3k_tq3_2v
            : mixed_kv
            ? ctx->device->pipeline_flash_attn_tq3k_tq2v
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                ...
                const bool mixed_kv_tq3k_tq2v = (op->src[1]->type == GGML_TYPE_TQ3_0 && op->src[2]->type == GGML_TYPE_TQ2_0);
                const bool mixed_kv_tq3k_tq3_2v = (op->src[1]->type == GGML_TYPE_TQ3_0 && op->src[2]->type == GGML_TYPE_TQ3_2);
                if (op->src[1]->type != op->src[2]->type && !mixed_kv_tq3k_tq2v && !mixed_kv_tq3k_tq3_2v) {
                    return false;
                }
                if (mixed_kv_tq3k_tq2v || mixed_kv_tq3k_tq3_2v) {
                    // mixed KV handled by dedicated pipeline
