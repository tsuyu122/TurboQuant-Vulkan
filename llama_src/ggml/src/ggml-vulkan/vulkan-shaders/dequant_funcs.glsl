...
// Extract a 3-bit index from the packed qs array at a given value position
uint tq3_extract_index(uint ib, uint val_pos, uint a_offset) {
    // 8 values packed into 3 bytes: group = val_pos / 8, within = val_pos % 8
...
// TQ3_2 uses TQ2_0 block format and 4-level codebook
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const float d = float(data_a[a_offset + ib].d);
    uint b = uint(data_a[a_offset + ib].qs[iqs >> 2u]);
    uint shift = (iqs & 2u) << 1u;
    uint idx0 = (b >> shift) & 3u;
    uint idx1 = (b >> (shift + 2u)) & 3u;
    return (d / 3.0) * vec2(float(int(idx0) * 2 - 3), float(int(idx1) * 2 - 3));
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const float d = float(data_a[a_offset + ib].d);
    uint b = uint(data_a[a_offset + ib].qs[iqs >> 2u]);
    ...
    uint idx2 = (b >> 4u) & 3u;
    uint idx3 = (b >> 6u) & 3u;
    return (d / 3.0) * vec4(float(int(idx0) * 2 - 3), float(int(idx1) * 2 - 3),
                            float(int(idx2) * 2 - 3), float(int(idx3) * 2 - 3));
}
#endif

#if defined(DATA_A_TQ3_3)
// TQ3_3: Same block layout as TQ3_2. Generic dequantize returns algebraic centroids
// (used only by non-FA code paths like plain dequant). Compute-aware Lloyd-Max
// remap + cross-quad filter is applied inside flash_attn_base.glsl.
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const float d = float(data_a[a_offset + ib].d);
    uint b = uint(data_a[a_offset + ib].qs[iqs >> 2u]);
    uint shift = (iqs & 2u) << 1u;
...
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const float d = float(data_a[a_offset + ib].d);
    // 4 values per byte; iqs is multiple of 2
    uint b = uint(data_a[a_offset + ib].qs[iqs >> 2u]);
    uint shift = (iqs & 2u) << 1u;  // 0 if iqs%4 < 2, else 4
    uint idx0 = (b >> shift) & 3u;
    uint idx1 = (b >> (shift + 2u)) & 3u;
    return d * vec2(float(int(idx0) - 1), float(int(idx1) - 1));
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const float d = float(data_a[a_offset + ib].d);
    // 4 consecutive values at iqs (multiple of 4) are packed in one byte
    uint b = uint(data_a[a_offset + ib].qs[iqs >> 2u]);
    uint idx0 =  b        & 3u;
    uint idx1 = (b >> 2u) & 3u;
    uint idx2 = (b >> 4u) & 3u;
    uint idx3 = (b >> 6u) & 3u;
    return d * vec4(float(int(idx0) - 1), float(int(idx1) - 1),
                    float(int(idx2) - 1), float(int(idx3) - 1));
}
#endif
