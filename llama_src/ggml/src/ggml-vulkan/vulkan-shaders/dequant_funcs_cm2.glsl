...
float16_t dequantFuncTQ3_0(const in decodeBufTQ3_0 bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    ...
    // Each group of 8 values is packed into 3 bytes (24 bits)
    const uint group = idx >> 3;        // 0..3
    const uint within = idx & 7;        // 0..7
    const uint byte_off = group * 3;    // byte offset: 0,3,6,9

    // Read 3 bytes from packed16 qs array
    uint b0, b1, b2;
...
layout(buffer_reference, std430, buffer_reference_align = 2) buffer decodeBufTQ3_2 {
   block_tq3_2_packed16 block;
};

float16_t dequantFuncTQ3_2(const in decodeBufTQ3_2 bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    const float16_t d = bl.block.d;
    const uint idx = coordInBlock[1];

    uint word_idx = idx >> 3u;
    uint w = uint(bl.block.qs[word_idx]);
    uint b = ((idx & 4u) == 0u) ? (w & 0xFFu) : ((w >> 8u) & 0xFFu);
    uint shift = (idx & 3u) << 1u;
    uint raw = (b >> shift) & 3u;

    return float16_t(int(raw) - 1) * d;           // {-1, 0, +1} * d
}
...
float16_t dequantFuncTQ2_0(const in decodeBufTQ2_0 bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    ...
    // 4 values per byte (2-bit indices); byte_idx = idx/4, bit_offset = 2*(idx%4)
    uint word_idx = idx >> 3u;            // which uint16 in qs[32]: idx/8
    uint w = uint(bl.block.qs[word_idx]);
    uint b = ((idx & 4u) == 0u) ? (w & 0xFFu) : ((w >> 8u) & 0xFFu);
    uint shift = (idx & 3u) << 1u;        // 0,2,4,6
    uint raw = (b >> shift) & 3u;         // {0,1,2} → {-1,0,+1}*d
    return float16_t(int(raw) - 1) * d;
}
...
#elif defined(DATA_A_TQ3_2)
#define dequantFuncA dequantFuncTQ3_2
